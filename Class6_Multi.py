...
class Class6_Multi(object):
    ...
    def set_model(self, learning_rate, range1):
        ...
        W1 = tf.Variable(tf.zeros([24, self.config.hidden_layer]))
        b1 = tf.Variable(tf.zeros([self.config.hidden_layer]))
        layer = tf.nn.sigmoid(tf.add(tf.matmul(x, W1), b1))
        W = tf.Variable(tf.zeros([self.config.hidden_layer, self.config.index]))
        b = tf.Variable(tf.zeros([self.config.index]))
        y = tf.nn.softmax(tf.matmul(layer, W) + b)
        ...
        plt.savefig('Multi_Index_'+str(self.config.index)+'_LearningRate_'+str(learning_rate)+"_Range_"+str(range1)+".jpg")
        plt.close()
        print("----------------------------------")
